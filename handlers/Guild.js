...
class Guild {
  ...
  isAllowedCategory(categoryId) {
    if (!Array.isArray(this.guildConfig.categories) || this.guildConfig.categories.length === 0) {
      return true
    }
  ...
  hasMentionedRole(message) {
    if (!Array.isArray(this.guildConfig.roles) || this.guildConfig.roles.length === 0) {
      return false
    }

    return message.mentions.roles.some((role) => this.guildConfig.mentions.includes(role.name))
  }
